 ...
 * 5. 만약, 간선이 (노드(정점)의 개수 - 1) 만큼 사용 되지 못했으면 스패닝을 이루지 못한 것입니다
 *
...
class MST_Kruskal {
    fun kruskal() {
        ...
        if (uf.cnt != v - 1) println(-1) // 만약 pq가 비었는데 노드 - 1개 만큼의 간선을 사용하지 못했다면 스패닝을 이루지 못한 것입니다
        else println(uf.result) // 최소 신장 트리의 가중치의 합을 출력합니다
    }
}
